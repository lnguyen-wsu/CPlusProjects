...
void ArrayList::del_Car(int id_del){
    ...
    for (int i = 0; i < size ; i++){
        if (pa[i].get_id() == id_del){    // compare array in the list
    ...
    // push Car to the left and start from the index + 1 postion
        for (int i = index + 1 ; i <size; i++){
            pa[i-1]= pa[i];      // assign the swapping value 
        }
...
void ArrayList::add_newMember(int index, Car temp){ 
